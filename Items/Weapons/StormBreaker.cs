...
namespace MTU.Items.Weapons
{
    class StormBreaker : ModItem
    {
        public override void SetStaticDefaults()
        {
            ...
            Tooltip.SetDefault("Hold right-click for lightning attack\n'A King's Weapon'");
        }
        ...
        public override bool CanUseItem(Player player)
        {
            if (player.altFunctionUse == 2) //Charge
            {
                ...
                item.shoot = ModContent.ProjectileType<StormbreakerProjectile>();
            ...
            return player.altFunctionUse != 2 || (player.ownedProjectileCounts[ModContent.ProjectileType<StormbreakerProjectile>()] <= 0);
        }
    ...
    {
        public override string Texture => "MTU/Items/Weapons/StormBreaker";
            ...
            else
            {
                if (_charge >= 50) //If charge is high enough, spawn lightning
                {
                    for (int i = 0; i < 3; ++i)
                    {
                        ...
                        int proj = Projectile.NewProjectile(projectile.position + dir * 60 - new Vector2(0, 16), Vector2.Zero, ModContent.ProjectileType<StormBreakerLightning>(), projectile.damage, 2f, projectile.owner);
                        Projectile pr = Main.projectile[proj];
                        pr.rotation = dir.ToRotation();

                        if (pr.modProjectile is StormBreakerLightning stormbreaker)
                        {
                            stormbreaker.constDir = dir;
                            stormbreaker.direction = dir;
                        }
                    }
                }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (_charge >= 50f) //Draw empowered StormBreaker (shows charge)
        ...
        /// <summary>Draws the lightning that strikes StormBreaker as it charges up</summary>
        private void DrawLightning(SpriteBatch spriteBatch)
        {
            Texture2D tex = Main.projectileTexture[ModContent.ProjectileType<StormBreakerLightning>()];
    ...
    public class StormBreakerLightning : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int proj = Projectile.NewProjectile(projectile.Center + (direction * 56), Vector2.Zero, ModContent.ProjectileType<StormBreakerLightning>(), projectile.damage, 2f, projectile.owner);
                Projectile pr = Main.projectile[proj];
                ...
                if (pr.modProjectile is StormBreakerLightning stormbreaker)
                {
                    stormbreaker.constDir = constDir;
                    stormbreaker.direction = newDir;
                }
            }
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit) => target.AddBuff(BuffID.Electrified, 60); //Inflict electrified on enemies
